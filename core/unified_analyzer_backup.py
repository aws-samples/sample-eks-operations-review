...
class UnifiedClusterAnalyzer:
    ...
    def analyze_dora(self) -> Dict[str, Any]:
        """Unified DORA analysis for both online and offline modes"""
        try:
            if self.is_offline:
                ...
                return dora_analyzer.run_comprehensive_dora_analysis()
            else:
                dora_analyzer = DORAComplianceAnalyzer(
                    cluster_name=self.cluster_name,
                    region=self.region,
                ...
                return dora_analyzer.run_dora_analysis_online()
        except Exception as e:
            logger.error(f"DORA analysis failed: {str(e)}")
            return {
                'error': f'DORA analysis failed: {str(e)}',
                'cluster_name': self.cluster_name,
                'dora_version': '2.0',
                'analysis_timestamp': datetime.now().isoformat(),
