...
class ReportGenerator:
    def get_real_cluster_metadata(self, aws_access_key, aws_secret_key, region, cluster_name):
        """Get real cluster metadata from AWS APIs"""
        try:
            from ..analyzers.cluster_state_analyzer import ClusterStateAnalyzer
            analyzer = ClusterStateAnalyzer(aws_access_key, aws_secret_key, region, cluster_name)
            cluster_state = analyzer.get_comprehensive_cluster_state()
    ...
    def generate_report(self, inputs, cluster_details, analysis_results, kb_insights=None, best_practices_recommendations=None):
        ...
        try:
            ...
            if best_practices_recommendations:
                story.append(PageBreak())
                self._add_best_practices_recommendations(story, best_practices_recommendations)
    ...
    def _add_best_practices_recommendations(self, story, best_practices_recommendations):
        """Add EKS best practices recommendations to the report"""
        story.append(Paragraph("EKS Best Practices Recommendations", self.styles['CustomHeading1']))
        story.append(Spacer(1, 0.1*inch))
        
        intro = """This section provides comprehensive recommendations based on the AWS EKS Best Practices Guide, 
        analyzed against your current cluster configuration. These recommendations cover areas beyond security 
        to include operational excellence, reliability, performance, and cost optimization."""
        ...
        for area, recommendation in best_practices_recommendations.items():
            if recommendation.get('applicable', False):
                # Area heading
                story.append(Paragraph(area, self.styles['CustomHeading2']))
                story.append(Spacer(1, 0.1*inch))
                
                # Priority indicator
                priority = recommendation.get('priority', 'Medium')
                priority_style = self.styles['RiskMedium']  # Default to medium
                if priority == 'High':
                    priority_style = self.styles['RiskHigh']
                elif priority == 'Low':
                    priority_style = self.styles['RiskLow']
                
                story.append(Paragraph(f"Priority: {priority}", priority_style))
                story.append(Spacer(1, 0.05*inch))
                
                # Analysis content
                analysis_text = recommendation.get('analysis', 'No analysis available')
        ...
        # If no applicable recommendations
        if not any(rec.get('applicable', False) for rec in best_practices_recommendations.values()):
            story.append(Paragraph("No additional best practices recommendations were identified for your current cluster configuration.", 
                                 self.styles['Normal']))
            story.append(Spacer(1, 0.1*inch))

    def _add_kb_insights(self, story, kb_insights):
        """Add knowledge base gap analysis insights to the report"""
        story.append(Paragraph("Knowledge Base Gap Analysis", self.styles['CustomHeading1']))
        story.append(Paragraph("Comparison of current cluster state against EKS best practices", self.styles['Normal']))
        story.append(Spacer(1, 0.1*inch))
        ...
        for pillar, fields in kb_insights.items():
            ...
            for field, insight in fields.items():
                ...
                # Parse the insight to format it better
                if isinstance(insight, str) and "**Status**" in insight:
                    # This is a gap analysis result - format it nicely
                    lines = insight.strip().split('\n')
                    for line in lines:
                        if line.strip():
                            clean_line = line.strip()
                            if clean_line.startswith('**') and clean_line.endswith('**'):
                                # Remove ** formatting and make it bold
                                ...
                                story.append(Paragraph(clean_line, 
                                                     ParagraphStyle(
                                                         name='GapDetail',
                                                         parent=self.styles['Normal'],
                                                         fontSize=10,
                    ...
                    # Regular insight text
                    story.append(Paragraph(insight, self.styles['Normal']))
                
                story.append(Spacer(1, 0.1*inch))
                    
    def _add_cluster_info(self, story, cluster_details):
        """Add cluster information section with real AWS data"""
        story.append(Paragraph("Cluster Information", self.styles['CustomHeading1']))
        story.append(Spacer(1, 0.1*inch))
        
        # Get real cluster metadata if available
        cluster_info = cluster_details.get('cluster', {})
        
        # Create a table for cluster information
        data = [
            ["Property", "Value"],
            ["Cluster Name", self._sanitize_log_input(cluster_info.get('name', 'N/A'))],
            ["Kubernetes Version", self._sanitize_log_input(str(cluster_info.get('version', 'N/A')))],
            ["Status", self._sanitize_log_input(str(cluster_info.get('status', 'N/A')))],
            ["Platform Version", self._sanitize_log_input(str(cluster_info.get('platform_version', 'N/A')))],
            ["Region", cluster_info.get('region', 'N/A')],
            ["Created At", cluster_info.get('created_at', 'N/A')],
            ["VPC ID", cluster_info.get('vpc_id', 'N/A')],
            ["Public Endpoint", "Enabled" if cluster_info.get('endpoint_public', False) else "Disabled"],
            ["Private Endpoint", "Enabled" if cluster_info.get('endpoint_private', False) else "Disabled"]
        ]
            ...
            if analysis_results.get(priority_key):
                ...
                for category, findings in sorted(findings_by_category.items()):
                    ...
                    for i, finding in enumerate(findings):
                        ...
                        # Current State row (if available)
                        if finding.get('current_state'):
                            data.append([Paragraph(f"<b>Current State:</b> {finding['current_state']}", self.styles['Normal'])])
